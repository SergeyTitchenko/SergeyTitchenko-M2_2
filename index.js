//Deposit, реализующий свойства и функциональность вклада, который хотел бы открыть клиент. 
...
//Проверяющая функция на соответствие введенных данных используется в callFunction
    checkFunction() {
...
//BankProduct реализующий свойства и функциональность банковского предложения по вкладу
...
//Calculator, инициализирующийся массивом продуктов BankProduct и вычисляющий наиболее выгодный вариант
class Calculator {
    constructor(deposit) {
        this.bankProductArr = [];
        this.deposit = deposit;
    }
//Перебирающая функция которую позже используем в callFunction
    initialFunction(obj) {
    ...
    filteredArray() {
        ...
        filteredArray = filteredArray.filter(bankProduct =>
            (this.deposit.firstAmount >= bankProduct.sumMin &&
            (this.deposit.firstAmount <= bankProduct.sumMax || bankProduct.sumMax == null)));
        filteredArray = filteredArray.filter(bankProduct =>
            (this.deposit.period >= bankProduct.termMin && this.deposit.period <= bankProduct.termMax));
...
//Application, реализующий обработку нажатия на кнопку, получение введенных данных и отображение результатов.
class Application {
    constructor() {
